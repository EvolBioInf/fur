...
#+begin_src latex
  ...
  think of all matches in the customary 5' to 3' direction.
#+end_src
...
#+end_src
#+begin_src latex
  When iterating over the results, every 5' match is compared to all 3'
  matches that could form an amplicion. Whenever an amplicon is found,
  the two primer accessions are printed together with the subject
  accession, the start and end positions of the amplicon on that
  subject, and the subject taxonomy.
#+end_src
