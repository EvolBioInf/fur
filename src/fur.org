...
#+begin_src latex
  ...
  GC-content, window length, and the inverse of the cumulative density
  ...
  \begin{figure}
    ...
    \caption{Cumulative density function (CDF) of the match complexity
      ...
      line indicates the complexity threshold for a cumulative value of
      0.95.}\label{fig:cdf}
  \end{figure}
#+end_src
...
#+begin_src latex
  ...
  complexity. We determine the start and end of the current window and
  decide whether or not it is inside a unique interval. A unique
  interval is closed by the first non-unique window that starts to the
  right of the interval, see $I_1$ in Figure~\ref{fig:sw}. However,
  after iterating across all windows, the start of the last window might
  still be inside the last interval, $I_2$ in Figure~\ref{fig:sw} is an
  example. So we check for this and store the last interval if needed.
#+end_src
#+begin_src C <<Sliding window analysis, P. \ref{fur}>>=
  ...
  while (fscanf(pp, "%f %f", &m, &c) != EOF) {
    ...
    //<<Window inside unique interval?, P. \ref{fur}>>
  }
  pclose(pp);
  if (in) {
    //<<Store last unique interval P. \ref{fur}>>
  }
#+end_src
#+begin_src latex
  ...
  complexity values, the interval array, and the start and end points of
...
#+end_src
#+begin_src latex
  We decide whether the current window is inside a unique interval or
  not.
#+end_src
#+begin_src C <<Window inside unique interval?, P. \ref{fur}>>=
  if (in) {
...
#+end_src
#+begin_src C <<Store last unique interval P. \ref{fur}>>=
  intvArrAdd(ia, newIntv(is, ie));
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src latex
  To mark the regions found in the neighbors, we open the BLAST output,
  scan it, and remove the BLAST output afterwards.
#+end_src
#+begin_src C <<Mark regions found among neighbors, P. \ref{fur}>>=
  ...
  fclose(fp);
  //<<Remove BLAST output file, P. \ref{fur}>>
#+end_src
#+begin_src latex
  For each line of BLAST output scanned, we get the query index, the
  query start, and its end. Then we set the query region to \ty{N}.
#+end_src
#+begin_src  C <<Scan output of neighbor BLAST, P. \ref{fur}>>=
  int qi, qs, qe;
  char s[32];
  while (fscanf(fp, "%s %d %d %d", s, &qi, &qs, &qe) != EOF) {
    //<<Set homologous neighbor regions to N, P. \ref{fur}>>
  }
#+end_src
#+begin_src latex
  If the template hasn't been deleted yet, we calculate the length of
  the region to be set to \ty{N}. If that region is greater than zero
  and the user asked for exact matches only, we delete the template. If
  that's not the case, we set the homologous positions to \ty{N},
...
#+end_src
#+begin_src latex
  We remove the BLAST output file.
#+end_src
#+begin_src C <<Remove BLAST output file, P. \ref{fur}>>=
  tmpl = "rm %s/o.txt";
  sprintf(cmd, tmpl, args->d);
  if (system(cmd) < 0) {
