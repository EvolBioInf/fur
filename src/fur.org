...
#+begin_src C <<Report result of sliding window analysis, P. \ref{fur}>>=
  ...
  for (int i = 0; i < ia->n; i++)
	...
	nn++;
  char *h1 = "# Step                    Sequences  Nucleotides  Mutations (N)";
  char *h2 = "# -------------------------------------------------------------";
  fprintf(stderr, "%s\n%s\n", h1, h2);
  tmpl = "# Sliding window             %6d     %8ld         %6d\n";
...
#+begin_src latex
  ...
  and absence from all neighbors.

  \section{Check Presence in All Targets}
  ...
  targets, the templates are intersected with the remaining
  targets. This is done using an external program,
  \texttt{phylonium}~\cite{klo20:phy}. \texttt{Phylonium} takes as input
  a set of sequences, one of which is designated the reference. In this
  case the reference is made up of the template candidates just
  identified. All contigs of the reference are concatenated. For
  example, in Figure~\ref{fig:int} the reference, $R$, consists of three
  unique candidate regions, $u_1$, $u_2$, and $u_3$. The remaining
  targets---$T_1$, $T_2$, and $T_3$ in the example---get aligned to
  $R$. Now region $u_1$ has overlapping matches from all three targets,
  region $u_2$ has matches from the three targets, but only those from
  $T_2$ and $T_3$ overlap, and region $u_3$ has no match from $T_3$. The
  intersection between $R$ and $T_1$--$T_3$ is formed by removing all
  positions with gaps, resulting in one template candidate, $c_1$.
...
#+end_src
#+begin_src latex
  The templates are written to the file \texttt{r.fasta} by iterating
  across the template array and printing headers and sequences.
#+end_src
...
#+begin_src latex
  ...
  individual files. The program \texttt{blastdbcmd}, which is part of
  the BLAST package, allows access to BLAST databases. This adds a blank
  to the end of headers, which we remove again to ensure accurate
...
#+end_src
#+begin_src latex
  When iterating across the BLAST database, neighbors and the
  target representative are avoided.
#+end_src
...
#+begin_src latex
  ...
  command, and save the results. These consist of a set of FASTA entries
  whose headers contain information about mutations in the fragment. The
  mutations are marked in the sequence to inform subsequent primer
  construction.
#+end_src
...
#+begin_src latex
  ...
  writes the intersection to the file \texttt{p.fasta}. All to
...
#+begin_src latex
  ...
  \texttt{N}, so they can later be avoided when designing primers. We
  also count the number of mutations to report them later.
#+end_src
...
#+end_src
#+begin_src latex
  \section{Check Absence from All Neighbors}
  The absence of the remaining templates among the neighbors is
  established using BLAST. The candidate templates are searched in the
  BLAST database and the hits written to file. This file is read back
  into the program, and the regions with homologs among the neighbors
  are again set to \texttt{N}.
#+end_src
...
#+begin_src latex
  ...
  coordinates to the output file, \texttt{o.txt}.
#+end_src
...
#+end_src
#+begin_src latex
  The maximum value an integer may take, \texttt{INT\_MAX}, is defined
  in \texttt{limits.h}.
#+end_src
#+begin_src C <<Include headers, P. \ref{fur}>>=
  #include <limits.h>
...
#+end_src
#+begin_src latex
  Finally, the regions with homology among the neighbors are set to
  \texttt{N}, bearing in mind that BLAST-coordinates are 1-based,
  character arrays 0-based. As the arrays with the start and end
  coordinates are not needed any more afterwards, they are freed.
#+end_src
...
#+end_src
#+begin_src latex
  In order to report the results of the absence test, we iterate over
  all residues in all sequences and count the number of mutations. At
  this point we can also classify the sequences into those fit for
  subsequent analysis and those that aren't.
#+end_src
...
#+end_src
#+begin_src latex
  The mutations are counted by looking at every residue in the current
  result set. An \texttt{N} is counted as a mutation, everything else
  as a nucleotide.
#+end_src
...
#+end_src
#+begin_src latex
  Sequences are classified as fit for printing if they contain at least
  as many nucleotides as the minimum length of a \texttt{phylonium}
  fragment.
#+end_src
