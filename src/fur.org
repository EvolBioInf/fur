#+begin_src latex
  ...
  the target strain to the genomes of the closest relatives it is to be
  distinguished from. The heuristic underlying this approach is that any
  region that distinguishes a target sequence from its neighborhood,
  ...
  \texttt{makeFurDb}\footnote{\texttt{https://github.com/haubold/makeFurDb/}}
    from two directories of sequence files, the
  first contains one or more \emph{target} genomes, the second one or
  more \emph{neighborhood} genomes. \texttt{fur} uses
  ...
  these two data structures, the program consists of the usual include
  section, declaration and definition of functions, and finally the
  \texttt{main} function.
#+end_src
#+begin_src C <<fur.c>>=
  #include "seq.h"
...
#+begin_src latex
  ...
  Interval arrays are one of the the basic building block of
...
#+end_src
#+begin_src latex
  The definition makes space for the new arrival and then adds it.
#+end_src
#+begin_src C <<Function definitions>>=
  void intvArrAdd(IntvArr *ia, Intv *i) {
    ia->arr = (Intv **)
...
#+begin_src latex
  ...
  Interval arrays are now ready to be used. This happens in the
  \texttt{main} function, which first interacts with the user, then
  analyzes the target and neighborhood sequences, and finally prints the
  desired template sequences. At the end of the program any memory still
  allocated is freed.
#+end_src
...
#+begin_src latex
  ...
  unique regions are checked for presence in all targets and absence
  from all neighborhood sequences.
#+end_src
...
#+begin_src latex
  ...
  \texttt{macle}~\cite{pir18:hig}. This operates by traversing a
  pre-computed index. The index is contained in the \texttt{fur}
  database and contains the neighborhood sequences augmented by the
  representative target sequence. The index is used to compute local
  complexity values. Based on these local complexity values, unique
  intervals are found.
#+end_src
...
#+end_src
#+begin_src latex
  To obtain the representative target sequence, its name is needed,
  which allows retrieval of the actual sequence.
#+end_src
...
#+end_src
#+begin_src latex
  With the sequence name as handles, the corresponding sequence is
...
#+begin_src latex
  ...
  unique intervals (dashed). In the algorithm the array of unique
  intervals is eventually converted into an array of unique sequences,
...
#+begin_src latex
  ...
  function (CDF) of the match length null
  distribution. Figure~\ref{fig:cdf} shows this function and how
  choosing a particular CDF-value on the $y$-axis corresponds to a
  complexity-threshold on the $x$-axis. Sequence length and GC content
  are looked up in the \texttt{macle} index, window length and
  probability supplied by the user.
...
#+end_src
#+begin_src latex
  The function \texttt{quantCm} is part of the \texttt{matchLen}
  library.
#+end_src
#+begin_src C <<Include headers>>=
  #include "matchLen.h"
...
#+begin_src latex
  ...
  positions from this, that is, 1 and 100, computed
  \begin{eqnarr*}
    \mbox{start} & = & \mbox{midPoint} - w / 2 + 1\\
    \mbox{end} & = & \mbox{midPoint} + w / 2
  \end{eqnarr*}
  Since positions in strings are zero-based, while \texttt{macle} output
  is one-based, the final start and end are shifted by one position to
  the left.
#+end_src
...
#+begin_src latex
  ...
  array of sequences. The templates are numbered and their length is
  included in the header. Once the templates have been written, the
...
#+begin_src latex
  ...
  checked first by searching template candidates and those templates not
  found in every target get deleted.
#+end_src
...
#+end_src
#+begin_src latex
  To search the target database, the target sequences are counted, the
  search pipe is constructed using this count, and the templates are
  written to it.
#+end_src
...
#+end_src
#+begin_src latex
  The target sequences are counted by parsing the sequences in the BLAST
  database.
#+end_src
...
#+begin_src latex
  ...
  target sequences. This is filtered by an AWK script to count the
  number of exact matches per template. The names of templates that
  match every target are written to file. Since \emph{exact} matches are
  required, the BLAST search is customized, such that it contains the
  query length in its output. Only hits where the query length is equal
  to the alignment length without mismatches are counted. Also, multiple
  hits to the same subject are counted only once. To facilitate the
  identification of templates, their indexes in the sequence array are
  used as FASTA identifiers. Indexes are numbers, so these get sorted to
  facilitate their subsequent handling. The output of the BLAST pipe is
  written to an output file located in the directory that also houses
  the \texttt{fur} database.
#+end_src
...
#+begin_src latex
  ...
  \texttt{blastn} pipe and then ending the neighborhood sequences down
  that pipe.
#+end_src
#+begin_src C <<Search neighborhood>>=
  //<<Construct neighborhood pipe>>
  //<<Write templates to neighborhood pipe>>
#+end_src
#+begin_src latex
  The command of the neighborhood pipe has three parts: Run BLAST, count
  the number of hits in the neighborhood, and write the names of the
...
#+begin_src sh <<Make fur database>>=
  makeFurDb -t target -n neighborhood -d furDb
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src latex
  The program lists five regions present in all targets, followed by two
  regions found in at least one of the neighborhood sequences. So the
  final number of sequences in \texttt{tmpl.fasta} is $5-2=3$. Use
  AWK to find that their combined length is 2.1 kb.
#+end_src
#+begin_src sh <<Analyze tutorial data>>=
  awk '/^>/{c++; s += $2} END {printf "%d tmp, %d bp\n", c, s}' tmpl.fasta
#+end_src
#+begin_src latex
...
#+begin_src latex
  ...
  templates with 5.6 kb. Clearly, \texttt{fur} is highly
...
#+begin_src latex
  ...
  (Figure~\ref{fig:cdf}). By default this is 0.95. Increasing it to 1
  abolishes all templates, but $P=0.999$ increases the yield to 29
  templates with 9.1 kb. So it might well be worth exploring different
  $P$-values and window lengths in other analyses. This can be done
  conveniently because each run of \texttt{fur} takes only a few seconds
  once the underlying database has been computed.
#+end_src
#+begin_src sh <<Refine tutorial analysis>>=
  fur -d furDb -w 250 -p 0.99999 |
      awk '/^>/{c++; s += $2} END {printf "%s tmp, %d bp\n", c, s}'
#+end_src
#+begin_src latex
