...
#+begin_src latex
  ...
  targets. This is done using a second external program,
  ...
  $R$. Region $u_1$ now has overlapping matches from all three targets,
...
#+begin_src C <<Report result of intersection, P. \ref{fur}>>=
  ...
  fprintf(stderr, tmpl, sa->n, nn, nm);
#+end_src
#+begin_src latex
...
#+begin_src latex
  ...
  neighbors are again set to \texttt{N}.
#+end_src
