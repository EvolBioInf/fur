...
#+begin_src latex
  ...
  decide whether or not it is inside a unique interval. After iterating
  across the windows we store the last unique interval we opened.
#+end_src
...
#+begin_src latex
  ...
  complexity values, the interval array, start and end points of windows
  and intervals, and a variable to indicate whether the program is
...
#+end_src
#+begin_src latex
  The last unique interval we opened still needs to be stored.
#+end_src
