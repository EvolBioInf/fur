#+begin_src latex
  ...
  \texttt{prim2fasta.awk} extracts primer pairs from \texttt{primer3}
  ...
  \begin{prog}[\texttt{prim2fasta.awk}]\label{prim2fasta}
    \textcolor{white}{}
#+end_src
#+begin_src awk <<prim2fasta.awk>>=
  #!/usr/bin/awk -f
  BEGIN {
...
#+begin_src awk <<Request base name, P. \ref{prim2fasta}>>=
  if (!file) {
    print "prim2fasta.awk: Extract primer sequences from primer3 output"
    print "Usage: prim2fasta.awk -v file=<fileBaseName> primer3.out"
    exit
  }
#+end_src
#+begin_src latex
