...
#+begin_src latex
  ...
  Database construction begins by reading the targets and the
  neighbors. Occasionally a user may duplicate a target among the
  neighbors or the other way round. This is always an error and we catch
  it.
#+end_src
#+begin_src C <<Read data, P. \ref{makeFurDb}>>=
  ...
  //<<Check targets and neighbors don't intersect, P. \ref{makeFurDb}>>
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src latex
  We make sure no sequences were duplicated between targets and
  neighbors.
#+end_src
#+begin_src C <<Check targets and neighbors don't intersect, P. \ref{makeFurDb}>>=
