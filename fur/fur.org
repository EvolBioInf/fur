...
which we set the number of threads and sensitivity thresholds: \ty{f}
for a minimum fraction of intersecting genomes, and a p-value for a
shusting length, which defines sensitivity of the exact matching in
the search for homologous regions. The user can restrict the output
from \ty{Intersect} to exact matches. She can again also print the
result of this step and exit.
#+end_export
...
the E-value, the number of threads (same as in the step
\ref{eq:fur2}), a switch to megablast mode instead of the default
blastn mode, and a switch for running with masking. We also set the
minimum length of the final regions at this point. The number of
threads is initialized to the number of CPUs.
#+end_export
...
\begin{itemize}
  ...
  \item \ty{PrintOneBased}: use one-based end-inclusive coordinates
    in the output;
  \item \ty{NumThreads}: number of concurrently processed files.
\end{itemize}
#+end_export
