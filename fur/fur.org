...
#+end_src
#+begin_export latex
Step~(\ref{eq:fur2}) finds regions present in a given fraction of
target genomes $f$, which is used as an intersection sensitivity
threshold. A calculation of a perfect intersection ($f = 1.0$, the
default value) is delegated to \ty{phylonium}, for which we don't
expose any options. However, the user can restrict the output from
phylonium to exact matches. A partial intersection ($0 < f < 1.0$) is
calculated with the imported function \ty{chr.Intersect}, for which we
expose $f$ and a p-value for a distribution of shustring lengths,
which are used to identify homology anchors in partial intersections.

The user can also print the result of Step~(\ref{eq:fur2}) and exit.
#+end_export
...
word length. We also set the minimum length of the final regions at
this point. The number of threads is initialized to the number of
CPUs.
#+end_export
...
#+end_src
#+begin_export latex
A sensitivity threshold $f$ is defined in $0 < f \le 1$. If the user
sets $f$ outside the range, we notify the user and exit.
#+end_export
...
#+end_src
#+begin_export latex
The user can override task-specific Blast word sizes by explicitly
setting it with the \ty{-W} flag. We visit all parsed flags and check
if \ty{-W} is provided. Nucleotide Blast allows the word size of 4 and
greater, so we check the provided value. If it is invalid, we politely
ask the user to set a valid one.
#+end_export
...
find an intersection, the implemetation of which depends on the
intersection sensitivity $f$:

\begin{itemize}
  \itemsep0em
  \item a perfect intersection ($f = 1.0$, which is the default): find
    regions present in all target genomes;
  \item a partial intersection ($0 < f < 1.0$): find regions present in a
    fraction $f$ of target genomes.
\end{itemize}

Like at the end of the previous step, we report its results and may be
also print them.
#+end_export
#+begin_src go <<Run step (\ref{eq:fur2}), Pr. \ref{pr:fur}>>=
  ...
  if numTargets > 1 {
	  if *optF == 1.0 {
		  //<<Find a perfect intersection, Pr. \ref{pr:fur}>>
	  } else {
		  //<<Find a partial intersection, Pr. \ref{pr:fur}>>
	  }
  }
...
#+end_src
#+begin_export latex
The search of a perfect intersection is implemented as a run of
\ty{phylonium}, which takes as input a file containing the unique
regions we just extracted. They serve as the reference, onto which
\ty{phylonium} piles the remaining target sequences. \ty{Phylonium}
writes its output to a file, from where we read it back into
\ty{fur}. We delete this output file and the input file of unique
regions again to leave the database as we found it.
#+end_export
#+begin_src go <<Find a perfect intersection, Pr. \ref{pr:fur}>>=
...
#+end_src
#+begin_export latex
If the user requested only exact matches, we skip any region with
polymorphic positions.  Otherwise, we set the polymorphic positions to
\ty{N}, so they can be avoided when designing primers.
#+end_export
...
end-exclusive. The region coordinates reported by the current version
of \ty{phylonium} are not guaranteed to correspond to region's actual
position in the reference genome if the latter contains nucleotides
other than \ty{A}, \ty{C}, \ty{G}, or \ty{T}. The most widespread
example of such a nucleotide is \ty{N}, which stands for "anything" in
genomic assemblies and is found in poorly resolved
regions. Non-\ty{ACGT} nucleotides are always cleaned out by
\ty{phylonium} resulting in left shifts in downstream regions of the
same contigs. After finding the intersection, \ty{phylonium} reports
region coordinates relative to the \textit{cleaned} reference, which
exists only in memory during runtime and may differ from the original
reference.

As the coordinates reported by \ty{phylonium} are not always robust,
we do not include them in the headers of the intersection
regions. However, we still report the reference contig name, the
region's id, and the number and positions of the mutations.

We iterate over the regions, extract header fileds, find the
corresponding reference contig, replace the header, and increase the
region's number.
#+end_export
#+begin_src go <<Adjust headers, Pr. \ref{pr:fur}>>=
  for regionNum, region := range regions {
	  //<<Extract header fields, Pr. \ref{pr:fur}>>
	  //<<Find reference intervals, Pr. \ref{pr:fur}>>
	  //<<Replace header, Pr. \ref{pr:fur}>>
  }
#+end_src
#+begin_export latex
We split the header at the parentheses. Then we split the interval at
the dot-dot. We also convert the positions we get from one-based to
zero-based and from end-exclusive to end-inclusive.
#+end_export
#+begin_src go <<Extract header fields, Pr. \ref{pr:fur}>>=
...
#+end_src
#+begin_export latex
To find reference intervals corresponding to the regions of the
intersection, we declare a variable for the index of the slice of
intervals in which we found a match, and a hook for additional
variables we might need when iterating over the intervals. Since
intervals are stored as slices of slices, we iterate over them in a
nested loop, from which we need to break whenever we've accomplished
the conversion. So we label the outer loop to eventually use a labeled
\ty{break} in our analysis of individual intervals. An additional
property of our search is that both the intervals returned in
step~(\ref{eq:fur1}) and the regions returned in step~(\ref{eq:fur2})
are ordered. This allows us to resume each search where we left off in
the previous round.
#+end_export
#+begin_src go <<Find reference intervals, Pr. \ref{pr:fur}>>=
  var ii, iii int
...
$m$. For a given interval $(s_{\rm i}..e_{\rm i})$ of length $l=e_{\rm
  ...
  p})$ intersects $(c..c+l-1)$. If so, the outer loop's iterator
\ty{ii} points at the reference contig we are looking for. We also
note that the unique region was found, increment $c$ by $l+1$, and
break out of the loop. If we don't break out of the loop, we just
increment $c$ by $l+1$.
#+end_export
...
old one. The new header consists of two elements:

\begin{enumerate}
  \itemsep0em
  \item the header of the appropriate representative sequence;
  \item the one-based number of the region;
\end{enumerate}

The first and the second element are joined by an underscore to
generate unique headers for the subsequent Blast step.

If, on the other hand, we didn't find the region, something is wrong
and we bail with message.
#+end_export
...
used by \ty{phylonium}. This concludes the implementation of the case
...
#+end_src
#+begin_export latex
The search of a partial intersection is implemented as a run of the
imported function \ty{chr.Intersect}. Just like \ty{phylonium}, it
also removes non-\ty{ACGT} nucleotides from the reference shifting the
downstream regions. Due to this, the coordinates of the regions we
have after Subtraction 1 become irrelevant, so we remove them from the
region's headers. Then we set the function's parameters and run
it. The function returns a slice of \ty{fasta} regions, which we use
to re-write the \ty{regions} variable. At the end of this step we
remove short regions and replace occasional exclamation marks with
\ty{N}.
#+end_export
#+begin_src go <<Find a partial intersection, Pr. \ref{pr:fur}>>=
  //<<Remove region coordinates from Subtraction 1, Pr. \ref{pr:fur}>>
  var parameters chr.Parameters
  //<<Set parameters for \ty{chr.Intersect}, Pr. \ref{pr:fur}>>
...
#+end_src
#+begin_export latex
We split regions' headers by the underscore followed by a parenthesis
and save the first field as new headers.
#+end_export
#+begin_src go <<Remove region coordinates from Subtraction 1, Pr. \ref{pr:fur}>>=
  for _, region := range regions {
...
\begin{enumerate}
  \itemsep0em
  \item a slice of \ty{fasta} regions, which we pass from the memory;
  \item a slice of paths to the queries, which we build using
    \ty{filepath.Glob} and \ty{-d};
  \item an intersection sensitivity threshold, which we pass from \ty{-f};
  \item a shustring length p-value for anchors, which we pass from \ty{-p};
  \item a number of threads, which we pass from \ty{-t};
  \item a switch to print mutations as \ty{N}s.
\end{enumerate}
#+end_export
#+begin_src go <<Set parameters for \ty{chr.Intersect}, Pr. \ref{pr:fur}>>=
...
#+end_src
#+begin_export latex
Again, the output regions' headers contain the reference contig name,
and the number of a unique region.

This concludes the implementation of the partial intersection.
#+end_export
#+begin_export latex
...
\[
\ty{>}\mbox{prefix}\_regionNumber\ n\ p_1\ p_2\ ...\ p_n
\]
When adjusting the header we need to adjust the mutations, which form
the header's suffix. So we extract the prefix and the suffix. Then
adjust the mutations and construct the new header.
#+end_export
...
number of threads to all threads available.
#+end_export
...
#+end_src
#+begin_export latex
We import \ty{runtime} and \ty{strconv}.
#+end_export
#+begin_src go <<Testing imports, Pr. \ref{pr:fur}>>=
  "runtime"
  "strconv"
...
#+end_src
#+begin_export latex
To test partial intersection, we use another database, in which one of
five targets lacks the marker. We set the sensitivity threshold to
0.8, or 4 target genomes.
#+end_export
#+begin_src go <<Test with partial intersection, Pr. \ref{pr:fur}>>=
  d = "testPartial.db"
  test = exec.Command("./fur", "-d", d, "-f", "0.8")  
  tests = append(tests, test)
#+end_src
#+begin_export latex
We run \ty{fur} in megablast and blastn modes setting the word size to
4 and 36, respectively. In the first case, we adjust megablast to be
actually more stringent than blastn, and in the second case, we adjust
both megablast and blastn to be less stringent than the default
megablast.
#+end_export
