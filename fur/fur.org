...
which we set a sensitivity threshold \ty{f}. The user can restrict the
output from \ty{Intersect} to exact matches. She can again also print
the result of this step and exit.
#+end_export
