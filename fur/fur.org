...
which we set the number of threads, which is initialized to the number
of CPUs. We proceed with sensitivity thresholds: \ty{f} for a minimum
fraction of intersecting genomes, and a p-value for a shusting length,
which defines sensitivity of the exact matching in the search for
homologous regions. The user can restrict the output from
\ty{Intersect} to exact matches. She can again also print the result
...
minimum length of the final regions at this point.
#+end_export
