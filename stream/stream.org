...
In \ty{makeFurDb} we compare pairs of sequences, $s_1$ and $s_2$, by
calculating an ESA of, say, $s_1$ and streaming $s_2$ against
it. Calculating an ESA of some sequence $s$ is also often called
\emph{indexing} $s$.
...
\begin{figure}
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Time (\textbf{A}) and memory \textbf{B} consumption as a
    function of sequence length under the indexing scenario (\emph{i})
  and the streaming scenario (\emph{s}).}\label{fig:strTime}
\end{figure}
...
#+end_src
#+begin_export latex
Inside \ty{scan}, we retrieve the arguments and iterate over the
sequences in the current file. For each sequence we construct the
missing sequences before we run the streaming.
#+end_export
...
#+begin_src go <<Run streaming, Pr. \ref{pr:str}>>=
  if *optI {
	  compare(s1fr, s2f, *optS)
  } else {
	  compare(s1f, s2fr, *optS)
  }
#+end_src
#+begin_export latex
Inside \ty{compare} we calculate the ESA of \ty{s1} and stream \ty{s2}
against it in steps determined by the skipping fraction.
#+end_export
#+begin_src go <<Functions, Pr. \ref{pr:str}>>=
  func compare(s1, s2 []byte, skip float64) {
	  e := esa.MakeEsa(s1)
	  i := 0
	  for i < len(s2) {
		  m := e.MatchPref(s2[i:])
...
#+end_src
#+begin_export latex
Our first test runs without options.
#+end_export
...
#+end_src
#+begin_export latex
The second test sets the seed for the random number generator.
#+end_export
#+begin_src go <<Construct tests, Pr. \ref{pr:str}>>=
  test = exec.Command("./stream", "-S", "3", f)
  tests = append(tests, test)
#+end_src
#+begin_export latex
The third test uses the indexing scenario.
#+end_export
#+begin_src go <<Construct tests, Pr. \ref{pr:str}>>=
...
#+end_src
#+begin_export latex
The fourth test doubles the default mutation rate from 1\% to 2\%.
#+end_export
#+begin_src go <<Construct tests, Pr. \ref{pr:str}>>=
  test = exec.Command("./stream", "-m", "0.02", f)
  tests = append(tests, test)
#+end_src
#+begin_export latex
The fourth test changes the skipping fraction from default zero to
0.5.
#+end_export
#+begin_src go <<Construct tests, Pr. \ref{pr:str}>>=
  test = exec.Command("./stream", "-s", "0.5", f)
  tests = append(tests, test)
#+end_src
#+begin_export latex
None of the tests produce a result so when we run them we just check
that they don't return an error message.
#+end_export
