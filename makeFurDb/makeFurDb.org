...
#+begin_src go <<Declare options, Pr. \ref{pr:mfd}>>=
  ...
  optQ := flag.Bool("q", false, "quiet mode")
#+end_src
#+begin_src latex
  We import \ty{flag}.
#+end_src
...
the three directory options, \ty{-t}, \ty{-n}, and \ty{-d}.
#+end_export
...
#+end_export
#+begin_src go <<Imports, Pr. \ref{pr:mfd}>>=
  "github.com/evolbioinf/ifur/util"
#+end_src
#+begin_export latex
...
other hand, the user did provide a name, the database might already
exist, in which case we deal with it. At the end we construct the empty
database directory that we populate in the following sections.
#+end_export
...
maps. However, map keys are accessed in random order. To impose order
on them, we sort the names before making sure they don't overlap. Then
we construct the six elements of the \ty{fur} database to store the
...
function \ty{readDir}. If either of the directories is empty, we bail
...
#+end_src
#+begin_export latex
A common error is to include a sequence file in both the target and
the neighbor set. To guard against this, we search for pairs of
repeated file names and bail with message as soon as we find one.
#+end_export
...
that make up the target representative, or rep. Then we allocate
corresponding space for the match lengths. We find the match lengths
by streaming the target rep against the neighbors. These neighbors may
be scattered over many short contigs. Naïvely, we might build one
suffix array per sequence, but it is more efficient to build a suffix
array for one long sequence than multiple suffix arrays for its
constituent substrings. So before we stream the target rep against a
neighbor suffix array, we determine the maximum neighbor
length. Double this maximum length is the upper bound of neighbor
nucleotides aggregated during the streaming step.
...
#+end_src
#+begin_export latex
To store the match lengths, we construct a slice of integer slices,
one for each target.
#+end_export
#+begin_src go <<Allocate space for match lengths, Pr. \ref{pr:mfd}>>=
  m := len(targetSeqs)
  matchLengths := make([][]int, m)
  for i := 0; i < m; i++ {
	  n := len(targetSeqs[i].Data())
	  matchLengths[i] = make([]int, n)
  }
#+end_src
#+begin_export latex
...
print a progress message for each sequence, construct the
corresponding suffix array, and stream the target rep against it. Note
that aggregation only works within a neighbor file, not across
multiple neighbors.
#+end_export
#+begin_src go <<Iterate over neighbor sequences, Pr. \ref{pr:mfd}>>=
  ...
  for sc.ScanSequence() {
	  ...
	  //<<Print progress message, Pr. \ref{pr:mfd}>>
...
#+end_src
#+begin_export latex
  As progress message we print up to 128 characters of the aggregate
  header of the aggregate neighbor being matched. If the header is
  longer than 128 characters, we append three dots. However, the user
  might have opted for a quiet run, in which case we skip this step.
#+end_export
#+begin_src go <<Print progress message, Pr. \ref{pr:mfd}>>=
    if !*optQ {
	    max := 128
	    n := len(h)
	    if n > max { n = max }
	    fmt.Fprintf(os.Stderr, "matching neighbor %s", h[:n])
	    if n < len(h) {
		    fmt.Fprintf(os.Stderr, "...")
	    }
	    fmt.Fprintf(os.Stderr, "\n")
...
#+end_src
#+begin_export latex
To complement the sequence, we first construct the dictionary for
complementing nucleotides, then we iterate over the sequence.
#+end_export
#+begin_src go <<Complement sequence, Pr. \ref{pr:mfd}>>=
  //<<Construct dictionary, Pr. \ref{pr:mfd}>>
  //<<Iterate over sequence, Pr. \ref{pr:mfd}>>
#+end_src
#+begin_export latex
The dictionary is only constructed on the first call of
\ty{revComp}. It complements the four canonical nucleotides and leaves
all other bytes unchanged.
#+end_export
#+begin_src go <<Construct dictionary, Pr. \ref{pr:mfd}>>=
  if dic == nil {
	  f := []byte("ACGT")
	  r := []byte("TGCA")
	  dic = make([]byte, 256)
...
#+end_src
#+begin_export latex
We iterate over the sequence to generate its complement.
#+end_export
#+begin_src go <<Iterate over sequence, Pr. \ref{pr:mfd}>>=
  for i, v := range d {
	  d[i] = dic[v]
  }
#+end_src
...
of zeros interspersed with ones. The line row requires special
attention.
#+end_export
...
#+end_src
#+begin_export latex
In our third and last test we also restrict the analysis to the
forward strand.
#+end_export
#+begin_src go <<Construct tests, Pr. \ref{pr:mfd}>>=
  test = exec.Command(p, "-t", a, "-n", n, "-d", d, "-o",
...
files \ty{r1.txt}, \ty{r2.txt}, and \ty{r3.txt}.
#+end_export
