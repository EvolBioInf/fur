...
#+end_src
#+begin_export latex
The default behavior is to skip forward by the length of the
match. This speeds up the program but reduces the accuracy of the
first subtraction. So the user can opt to skip a fraction of the match
by setting \ty{-s} to a floating point number greater than zero, or by
a single nucleotide by setting \ty{-s} to a negative number.
#+end_export
#+begin_src go <<Advance match position, Pr. \ref{pr:mfd}>>=
  if *optS == 0 {
	  j += m.L
  } else if *optS > 0 {
	  j += int(float64(m.L) * *optS) + 1
  } else {
	  j++
  }
#+end_src
...
#+end_export
#+begin_src go <<Declare options, Pr. \ref{pr:mfd}>>=
  optS := flag.Float64("s", 0, "skip by fraction of match " +
	  "(default all, < 0 for single base)")
#+end_src
#+begin_export latex
