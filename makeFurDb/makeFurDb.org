...
neighbor sequences and convert each one to a Blast database, which we
number, hence we also declare a counter, $c$. To facilitate subsequent
handling of the databases, we then link them into one entity.
#+end_export
...
#+end_src
#+begin_export latex
We need to preserve any masking information contained in the Blast
database. We do this by first extracting the mask and the applying it
when constructing the database with \ty{makeblastdb}.
#+end_export
...
#+end_src
#+begin_export latex
We run \ty{makeblastdb}, which is also part of the Blast package. To
ensure each database has a unique name, we call then \ty{n}$c$, where
$c$ is iterated in each round.
#+end_export
...
the another program from the Blast package, \ty{blastdb\_aliastool}.
#+end_export
...
used by \ty{fur} to calculate the null distribution of match
...
with the number of nucleotides, which we call the sequence length.
#+end_export
