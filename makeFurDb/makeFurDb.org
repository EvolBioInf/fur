...
#+end_src
#+begin_export latex
Our cleaning step is quite brief, it doesn't even affect the sequence
data. We just reduce the header to the first token, usually the
accession. This ensures that there are no blanks in the names of the
sequences later passed to Blast.
#+end_export
...
neighbor sequences and convert each one to a Blast database. To
facilitate subsequent handling of the databases, we then link them
into one entity.
#+end_export
...
#+end_src
#+begin_export latex
We make the Blast database from the current sequence file by calling
the program \ty{makeblastdb}, which is part of the Blast package.
#+end_export
...
#+end_src
#+begin_export latex
We import \ty{strconv} and \ty{exec}.
#+end_export
#+begin_src go <<Imports, Pr. \ref{pr:mfd}>>=
  "strconv"
  "os/exec"
