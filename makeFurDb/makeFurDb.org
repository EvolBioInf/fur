...
forward one position and repeat.

In earlier versions of this algorithm I skipped forward by the length
of the match. This speeds up the program but drastically reduces the
accuracy of the first subtraction. Perhaps some compromise like
skipping half the match can be found in the future.
#+end_export
#+begin_src go <<Functions, Pr. \ref{pr:mfd}>>=
  func streamSeq(ni []byte, resa *esa.Esa, ml []int) {
	  for j := 0; j < len(ni); j++ {
